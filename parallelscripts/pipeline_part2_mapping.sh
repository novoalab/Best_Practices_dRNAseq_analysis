...
gzip -d $fastq_file
awk '{ if (NR%4 == 2) {gsub(/U/,"T",$1); print $1} else print }' ${fastq_file%/*}/${name%.gz} > ${fastq_file%/*}/${name%.fastq.gz}_UtoT.fastq
gzip ${fastq_file%/*}/${name%.fastq.gz}_UtoT.fastq
