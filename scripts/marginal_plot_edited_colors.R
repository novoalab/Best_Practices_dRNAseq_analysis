marginal_plot = function(x, y, group = NULL, data = NULL, lm_formula = y ~ x, bw = "nrd0", alpha = 1, plot_legend = T, ...){
  ...
  if(missing(group)){
    ...
    group_colors = rainbow(length(unique(data$group)) + 1)[-2] #When there are only 2 colours, we get a very light blue, in this way, we get a darker blue
    #group_colors = c(rgb(1,0.5,0.5,0.5),rgb(0,0.5,1,0.5),rainbow(length(unique(data$group))))
  } 
  ...
  tryCatch(expr = {
    ...
    if(!missing(group) & plot_legend){
      legend("center", levels(data$group), fill = group_colors, border = group_colors, bty = "n", title = deparse(substitute(group)), title.adj = 0.25)
    }
